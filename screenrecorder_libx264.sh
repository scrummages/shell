...
if [ "$#" -eq 0 ]; then
    echo "No CRF provided. Setting to 30"
else
    echo "CRF: $@"
    crf=$@
fi
...
combine_audio_video() {
    ...
    ffmpeg -i "$video_file" -i "$audio_file" -c:v libx264 -crf 23 -preset medium -c:a aac -strict experimental -b:a 192k "$output_file"
...
record_audio_video() {
    ...
    ffmpeg -y \
        ...
        -c:v libx264 -crf $crf -preset medium -r 30 \
        -c:a flac \
        "$video_file" &
